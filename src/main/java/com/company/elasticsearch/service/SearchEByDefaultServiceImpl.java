...
public class SearchEByDefaultServiceImpl implements SearchEByDefaultService, InitializingBean {
	...
	protected ProcessResult searchCourseByArray(String searchContentArray, SearchRequest queryPageRequest,
			String queryColumnFiled[]) {
		...
		if (!StringUtils.isEmpty(searchContentArray)) {
			BoolQueryBuilder contentQueryBuilder = QueryBuilders.boolQuery();

			// boolQueryBuilder.should(QueryBuilders.multiMatchQuery(searchContentArray[i],
			// columnFiled));
			// for(int j=0;j<queryColumnFiled.length;j++)

			logger.debug("query content:" + ":" + "*" + searchContentArray.trim() + "*");
			// contentQueryBuilder.should(QueryBuilders.wildcardQuery(queryColumnFiled[j],
			// "*" + searchContentArray[i] + "*"));
			contentQueryBuilder.should(QueryBuilders.multiMatchQuery(searchContentArray, queryColumnFiled));
		...
		if (queryPageRequest.getPriceStart() >= 0
				&& queryPageRequest.getPriceStart() <= queryPageRequest.getPriceEnd()) {
			logger.debug("query by price:" + queryPageRequest.getPriceStart() + ":" + queryPageRequest.getPriceEnd());
			boolQueryBuilder.must(QueryBuilders.rangeQuery("realPrice").gte(queryPageRequest.getPriceStart())
					.lte(queryPageRequest.getPriceEnd()));
			isMatchQuery = true;
		}
		if (!isMatchQuery) {
		...
		/*
		 * FilterFunctionBuilder[] lists = new
		 * FilterFunctionBuilder[5*searchContentArray.length];
		 * 
		 * 
		 * for(int i=0;i<searchContentArray.length;i++) { String searchContent =
		 * searchContentArray[i];
		 * 
		 * 
		 * 
		 * lists[5*i] = new
		 * FilterFunctionBuilder(QueryBuilders.boolQuery().must(QueryBuilders.matchQuery
		 * ("searchKeys", searchContent)),
		 * ScoreFunctionBuilders.fieldValueFactorFunction("totalRank")); lists[5*i+1] =
		 * new
		 * FilterFunctionBuilder(QueryBuilders.boolQuery().must(QueryBuilders.matchQuery
		 * ("category", searchContent)),
		 * ScoreFunctionBuilders.fieldValueFactorFunction("totalRank")); lists[5*i+2] =
		 * new
		 * FilterFunctionBuilder(QueryBuilders.boolQuery().must(QueryBuilders.matchQuery
		 * ("title", searchContent)),
		 * ScoreFunctionBuilders.fieldValueFactorFunction("totalRank")); lists[5*i+3] =
		 * new
		 * FilterFunctionBuilder(QueryBuilders.boolQuery().must(QueryBuilders.matchQuery
		 * ("courseInfo", searchContent)),
		 * ScoreFunctionBuilders.fieldValueFactorFunction("totalRank")); lists[5*i+4] =
		 * new
		 * FilterFunctionBuilder(QueryBuilders.boolQuery().must(QueryBuilders.matchQuery
		 * ("teacherName", searchContent)),
		 ...
		 * .add(QueryBuilders.boolQuery().should(QueryBuilders.matchQuery("searchKeys",
		 * searchContent)), ScoreFunctionBuilders.fieldValueFactorFunction("totalRank"))
		 * .add(QueryBuilders.boolQuery().should(QueryBuilders.matchQuery("category",
		 * searchContent)), ScoreFunctionBuilders.fieldValueFactorFunction("totalRank"))
		 * .add(QueryBuilders.boolQuery().should(QueryBuilders.matchQuery("title",
		 * searchContent)), ScoreFunctionBuilders.fieldValueFactorFunction("totalRank"))
		 * .add(QueryBuilders.boolQuery().should(QueryBuilders.matchQuery("courseInfo",
		 * searchContent)), ScoreFunctionBuilders.weightFactorFunction(100))
		 * .add(QueryBuilders.boolQuery().should(QueryBuilders.matchQuery("teacherName",
		 * searchContent)), ScoreFunctionBuilders.weightFactorFunction(100));
		 */
		// 创建搜索 DSL 查询
	...
	public ProcessResult searchCourse(String searchContent, SearchRequest queryPageRequest) {
		...
		return this.searchCourseByArray(searchContent.trim().toLowerCase(), queryPageRequest,this.columnFiled);
	}
	...
	public ProcessResult updateTotalRank(String courseId, int totalRank) {
		...
		UpdateQuery updateQuery = new UpdateQueryBuilder().withId(courseId).withClass(CourseSearch.class)
	...
	public static String getNewValue(Object object, String propertyName, String offset) throws ProcessResultException {
		...
		if (objectValue == null) {
			...
			throw new ProcessResultException(
					ControllerUtils.getErrorResponse(-1, "parameter " + propertyName + " value is null"));
		}
